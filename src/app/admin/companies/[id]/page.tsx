...
export default function CompanyDetailPage({
    ...
    async function fetchCompanyData() {
      try {
        ...
        if (data.company) {
          ...
          setIsPublished(data.company.isPublished || false);
        }
      } catch (error) {
        console.error("Error fetching company data:", error);
        // モックデータにフォールバック
        const companyData = MOCK_COMPANY_DATA[id];
        if (companyData) {
  ...
  async function handleDelete() {
    if (
      !confirm(
        "この工務店を削除してもよろしいですか？関連するメンバー、施工事例、問い合わせもすべて削除されます。"
      )
    ) {
      return;
    }
    ...
    try {
      ...
      alert(
        error instanceof Error ? error.message : "工務店の削除に失敗しました"
      );
    }
  }
          ...
          <div className="flex flex-1 justify-between px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg transition border border-red-200"
              >
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
              ...
              {mainImageUrl ? (
                <div className="space-y-4">
                  ...
                  >
                    <Trash2 className="h-4 w-4" />
                    <span className="font-medium">メイン画像を削除</span>
                  </button>
                </div>
              ) : (
                <TempImageUploader
